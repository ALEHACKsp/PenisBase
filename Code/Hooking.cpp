...
namespace Big
{
	...
	bool Hooks::IsDlcPresent()
	{
		...
		return static_cast<decltype(&IsDlcPresent)>(g_Hooking->m_OriginalIsDlcPresent)();
	}

	const char* Hooks::GetNasATL(void* unk, const char* label)
	{
		return "Penis";

		return static_cast<decltype(&GetNasATL)>(g_Hooking->m_OriginalGetNasATL)(unk, label);
	}
		...
		m_ObjectBypass(Signature("74 48 E8 ? ? ? ? 48 8B 48 48").Scan().Add(3).Rip().Add(30).As<char*>())
	{
		...
		MH_CreateHook(g_GameFunctions->m_GetLabelText, &Hooks::GetNasATL, &m_OriginalGetNasATL);
